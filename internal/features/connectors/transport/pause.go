...
func (h *ConnectorsHTTPHandler) PauseConnector(w http.ResponseWriter, r *http.Request) {
	const op = "connectors.transport.PauseConnector"
	...
	connector, err := h.connectorsService.PauseConnector(ctx, connectorID)
	if err != nil {
		responseHandler.ErrorResponse(fmt.Errorf("%s: pause connector: %w", op, err))
